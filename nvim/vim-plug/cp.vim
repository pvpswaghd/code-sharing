...
command! -nargs=1 -complete=file CompileAndRunWithFile call TermWrapper(printf('g++ -pedantic-errors -std=c++11 %s && ./a.out < %s', expand('%'), <q-args>))
command! -nargs=0 CompilePython call TermWrapper(printf('python3 %s', expand('%')))
autocmd FileType python nnoremap <leader>fe :CompilePython<CR>
augroup cppRun
	autocmd!
...
"let g:split_term_resize_cmd = 'default'
