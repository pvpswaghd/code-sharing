...
bool operator<(const User & a, const User & b) {
  return (a.size() < b.size()); //temporary
}

bool cmp_page_id(const Page &a, const Page &b){
  return (a < b);
}
// Please implement the following function
void add_page(const Page& p) {
  pages.push_back(p); 
  //cout << "Page: " << p.id << " " << p.path << " added" << endl;
  sort(pages.begin(), pages.end(), cmp_page_id);
}

// Please implement the following function
bool cmp_page_count(const Page & a, const Page & b) {
  if(a.counter == b.counter) return (a.path < b.path);
  return (a.counter > b.counter); //temporary
}

bool cmp_user_size(const User &a, const User & b){
...
int main() {
    ...
    sort(users.begin(), users.end(), cmp_user_size);
